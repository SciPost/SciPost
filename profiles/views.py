...
class ProfileDuplicateListView(PermissionsMixin, PaginationMixin, ListView):
    ...
    def get_queryset(self):
        qs = Profile.objects.annotate(full_name=Concat('last_name', 'first_name'))
        duplicates = qs.values('full_name').annotate(
            nr_count=Count('full_name')).filter(nr_count__gt=1).values_list('full_name', flat=True)
        return qs.filter(full_name__in=duplicates).order_by('last_name', '-id')
