...
class JournalAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'ISSN_digital', 'last_full_sync', 'count_metacore', 'count_crossref', 'last_update', 'task_status')
    actions = ['import_full', 'scheduled_import_incremental', 'import_incremental', 'update_counts', 'add_journal_to_items', 'delete_all_citables']
    ...
    def import_full(self, request, queryset):
        ...
        for journal in queryset:
            ...
            messages.add_message(request, messages.INFO, 'Import task for journal {} added.'.format(journal.name))
    ...
    def import_incremental(self, request, queryset):
        ...
        for journal in queryset:
            if journal.last_full_sync:
                t = import_journal_incremental.delay(journal.ISSN_digital, journal.last_full_sync.strftime('%Y-%m-%d'))
                journal.last_task_id = t.id
                journal.save()
                messages.add_message(request, messages.INFO, 'Import task for journal {} added.'.format(journal.name))
            else:
                messages.add_message(request, messages.WARNING, 'Incremental import task for journal {} could not be started, since date of last full sync is not set.'.format(journal.name))
    ...
    def scheduled_import_incremental(self, request, queryset):
        ...
        for journal in queryset:
            if journal.last_full_sync:
                ...
                t = PeriodicTask.objects.create(interval=schedule, name='Inc. import {}'.format(journal.name), 
                        task='metacore.services.import_journal_incremental', args=json.dumps([journal.ISSN_digital, 
                        journal.last_full_sync.strftime('%Y-%m-%d')]))

                #TODO: figure out a way to put the individual task id in the journal
                # everytime the scheduled task fires
                journal.last_task_id = ''
                journal.save()
                messages.add_message(request, messages.INFO, 'Repeating import task for journal {} added. Go to Periodic Tasks in admin to view'.format(journal.name))
            else:
                messages.add_message(request, messages.WARNING, 'Incremental import task for journal {} could not be started, since date of last full sync is not set.'.format(journal.name))
