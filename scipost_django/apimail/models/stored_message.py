...
def get_mime_upload_path(instance, filename):
    ...
    if not isinstance(instance.uuid, uuid_lib.UUID):
        raise FieldError(
            "StoredMessage MIME upload path cannot be determined (uuid not defined)."
        )
    u = str(instance.uuid)
...
class StoredMessage(models.Model):
    ...
    tags = models.ManyToManyField(
        "apimail.UserTag", blank=True, related_name="messages"
    )
    ...
    mime = models.FileField(
        ...
        validators=[
            validate_max_email_mime_file_size,
        ],
    ...
    def get_absolute_url_api(self):
        return reverse(
            "apimail:api_stored_message_retrieve", kwargs={"uuid": self.uuid}
        )
