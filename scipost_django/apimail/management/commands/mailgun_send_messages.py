...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        for msg in emails_ready_to_send:
            ...
            files = [
                ("attachment", (att.data["name"], att.file.read()))
                for att in msg.attachment_files.all()
            ]

            response = requests.post(
                "https://api.eu.mailgun.net/v3/%s/messages"
                % msg.from_account.domain.name,
            ...
            msgr = ComposedMessageAPIResponse(
                message=msg, status_code=response.status_code, json=response.json()
            )
            msgr.save()

            if response.status_code == 200:
                ComposedMessage.objects.filter(uuid=msg.uuid).update(
                    status=ComposedMessage.STATUS_SENT
                )
