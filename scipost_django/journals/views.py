...
def reset_author_associations(publication_object: Publication) -> None:
    ...
    author_string_list, author_affiliation_ids = fetch_tex_author_info(publication_object)    
...
def add_author(request, doi_label: str) -> HttpResponse:
    ...
    if request.POST and form.is_valid():
        ...
        return render(request, "journals/add_author_list.html", {"author_list": author_render_list, "selected_author_id": int(request.POST["selected_author_id"])})
...
def search_for_authors(publication_object: Publication, author_string_list: list[str]) -> list:
...
def build_author_render_list(author_string_list: list[str], author_profile_query_list: list, author_affiliation_ids: list[list[int]]) -> list:
    """ Creates a list of dictionaries for each author. This list is used to render the author list in the add_author view."""
    author_dict_lsit = []
    
    for author_profile_query, tex_author, their_affiliations in zip(author_profile_query_list, author_string_list, author_affiliation_ids): # Author1, Author2, ...
        ...
        author_dict_lsit.append({"tex_name": tex_author, "profile": author_profile, "affiliations": their_affiliations,
...
def fetch_tex_author_info(publication: Publication) -> tuple[list[str], list[list[int]]]:
    ...
    if request.POST:
        if not request.POST.get("submit"):
            ...
            if total_non_empty_affiliations != total_affiliations_from_tex:
                ...
                authors_list, affiliations_of_authors = fetch_tex_author_info(publication)
