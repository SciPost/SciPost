...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--issue_doi_label",
            choices=[issue.doi_label for issue in Issue.objects.all()],
            action="store",
            dest="issue_doi_label",
    ...
    def handle(self, *args, **kwargs):
        ...
        if ("/%s/%s" % (issue.in_volume.number, issue.number)) not in issue.path:
            print(
                "WARNING: /%s/%s not in issue path %s"
            ...
            for pub in issue.publications.all():
                ...
                old_dir = old_issue_path + "/{paper_nr}".format(
                ...
                new_pub_path = new_dir + "/{doi}.pdf".format(
                ...
                for deposit in pub.deposit_set.all():
                    new_dep_path = new_dir + "/{doi}_Crossref_{timestamp}.xml".format(
                        doi=pub.doi_label.replace(".", "_"), timestamp=deposit.timestamp
                    )
                    os.rename(
                        deposit.metadata_xml_file.path,
                        settings.MEDIA_ROOT + new_dep_path,
                    )
                ...
                old_dep_path = old_dir + "/{doi}_Crossref.xml".format(
                    paper_nr=pub.get_paper_nr(), doi=pub.doi_label.replace(".", "_")
                )
                new_dep_path = new_dir + "/{doi}_Crossref.xml".format(
                    paper_nr=pub.get_paper_nr(), doi=pub.doi_label.replace(".", "_")
                )
                os.rename(
                    settings.MEDIA_ROOT + old_dep_path,
                    settings.MEDIA_ROOT + new_dep_path,
                )
