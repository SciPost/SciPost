...
class Issue(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=ISSUE_STATUSES, default=STATUS_PUBLISHED
    )
    doi_label = models.CharField(
        max_length=200, unique=True, db_index=True, validators=[doi_issue_validator]
    )
    ...
    def clean(self):
            ...
            raise ValidationError(
                {
                    "in_journal": ValidationError(
                        "Either assign a Journal or Volume to this Issue",
                        code="required",
                    ),
                    "in_volume": ValidationError(
                        "Either assign a Journal or Volume to this Issue",
                        code="required",
                    ),
                }
        ...
        if self.in_journal and not self.in_journal.has_issues:
            raise ValidationError(
                {
                    "in_journal": ValidationError(
                        "This journal does not allow for the use of Issues",
                        code="invalid",
                    ),
                }
            )
    ...
    def period_as_string(self):
        if self.start_date.month == self.until_date.month:
            return "%s %s" % (
                self.until_date.strftime("%B"),
                self.until_date.strftime("%Y"),
            )
        return "%s - %s" % (
            self.start_date.strftime("%B"),
            self.until_date.strftime("%B %Y"),
        )
    ...
    def avg_processing_duration(self):
        ...
        duration = Publication.objects.filter(in_issue=self).aggregate(
