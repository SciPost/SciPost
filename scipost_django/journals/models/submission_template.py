...
def submission_template_upload_path(instance, filename):
    return "uploads/templates/{0}/{1}/{2}".format(
        instance.journal.doi_label, instance.date.year, filename
    )
...
class SubmissionTemplate(models.Model):
    ...
    journal = models.ForeignKey(
        "journals.Journal", on_delete=models.CASCADE, related_name="templates"
    )

    template_type = models.CharField(
        max_length=32, choices=TYPE_CHOICES, default=TYPE_LATEX_TGZ
    )

    template_file = models.FileField(
        upload_to=submission_template_upload_path, max_length=256
    )

    date = models.DateField()

    instructions = models.TextField(default="[To be filled in; you can use markup]")
