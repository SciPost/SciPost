...
def celery_fetch_potential_coauthorships_for_profile_and_submission_authors(
    ...
    **kwargs: dict[str, Any],
    ...
    print(
        f"Checking potential coauthorships for profile {profile_id} and "
        f"{len(submission_authors)} submission authors "
        f"across {len(preprint_servers)} preprint servers."
    )
    ...
    for coauthor in submission_authors:
        ...
        for preprint_server in preprint_servers:
            ...
            self.update_state(
                state="PROGRESS",
                meta={
                    "progress": round(nr_pairs_checked / nr_total_pairs, 3),
                },
            )
            print(f"Checked {nr_pairs_checked}/{nr_total_pairs} pairs")
    ...
    **kwargs: dict[str, Any],
    ...
    for i, preprint_server in enumerate(preprint_servers, 1):
        ...
        self.update_state(
            state="PROGRESS",
            meta={
                "progress": int(i / len(preprint_servers)),
            },
        )
