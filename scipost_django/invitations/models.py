...
class RegistrationInvitation(models.Model):
    ...
    status = models.CharField(
        max_length=8,
        choices=constants.REGISTATION_INVITATION_STATUSES,
    ...
    message_style = models.CharField(
        max_length=1,
        choices=constants.INVITATION_STYLE,
    ...
    invitation_type = models.CharField(
        max_length=2,
        choices=constants.INVITATION_TYPE,
    ...
    def __str__(self):
        return "{} {} on {}".format(
            self.first_name, self.last_name, self.created.strftime("%Y-%m-%d")
        )
...
class CitationNotification(models.Model):
    ...
    def related_notifications(self):
        return CitationNotification.objects.unprocessed().filter(
            models.Q(contributor=self.contributor)
            | models.Q(invitation=self.invitation)
        )
