...
def merge_submission_statuses(apps, schema_editor):
    ...
    Submission.objects.filter(
        status__in=["resubmitted_and_rejected", "resubmitted_and_rejected_visible"]
    ...
    ).update(status="recommendation_formulated")

    # Closed formulated Submissions
    Submission.objects.filter(status="review_closed").update(status="awaiting_ed_rec")

    # Incoming Submissions
    Submission.objects.filter(status__in=["unassigned", "resubmitted_incoming"]).update(
        status="unassigned_incoming"
    )
