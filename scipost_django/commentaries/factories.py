...
class BaseCommentaryFactory(factory.django.DjangoModelFactory):
    ...
    arxiv_identifier = factory.Sequence(
        lambda n: random_arxiv_identifier_with_version_number("1")
    )
    # arxiv_link
    pub_DOI = factory.Sequence(lambda n: random_external_doi())
    # pub_DOI_link
    # metadata
    arxiv_or_DOI_string = factory.lazy_attribute(
        lambda o: (o.arxiv_identifier if o.arxiv_identifier else o.pub_DOI)
    )
    # scipost_publication
    ...
    def add_authors(self, create, extracted, **kwargs):
        contributors = Contributor.objects.order_by("?").exclude(
            pk=self.requested_by.pk
        )[:4]
