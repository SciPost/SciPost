        ...
        if c.profile:
            ...
            if c.personalwebpage:
                Profile.objects.filter(
                    pk=c.profile.id).update(webpage=c.personalwebpage)
            if not c.accepts_SciPost_emails:
                Profile.objects.filter(
                    pk=c.profile.id).update(accepts_SciPost_emails=False)
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(transfer_from_contributor_to_profile,
